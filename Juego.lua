...
function Juego:init()
    ...
    self.mapa = love.graphics.newImage(
        "assets/mapa.png"
    )
...
function Juego:obtenerDatosMapa()
    ...
    local mapaX =
        (anchoPantalla - anchoMapa * escala) / 2

    local mapaY =
        (altoPantalla - altoMapa * escala) / 2
...
function Juego:actualizar(dt)
    ...
    if self.jugador.x + self.jugador.radio >
       anchoPantalla then
    ...
    if self.jugador.y + self.jugador.radio >
       altoPantalla then
    ...
    if self.gestorOleadas.numeroOleada <
       self.maxOleadas then
    ...
    if self.gestorOleadas.numeroOleada >= self.maxOleadas and #self.enemigos == 0 and self.gestorOleadas.estado ~= "oleada" then
        ...
        print(
            "VICTORIA: Eliminaste las 5 oleadas"
        )
    end
end
...
function Juego:dibujar()
    ...
    love.graphics.print("ESPACIO  -  Atacar",panelX + 15,panelY + 6)
